...
class HomeLedgerApiClient:
    ...
    def _build_payload(self, response: dict[str, Any]) -> dict[str, Any]:
        ...
        total_electricity_cost = round(seed * 1.25, 2)
        total_gas_cost = round(seed * 0.85, 2)
        total_water_cost = round(seed * 0.35, 2)
        total_electricity_consumption = round(seed * 12.5, 2)
        total_gas_consumption = round(seed * 1.75, 2)
        total_water_consumption = round(seed * 0.5, 2)

        return {
            "model": "Home Ledger",
            "serial_number": f"HL-{seed:06d}",
            "sw_version": "1.4.2",
