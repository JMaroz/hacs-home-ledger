...
from custom_components.home_ledger.api import HomeLedgerApiClientAuthenticationError, HomeLedgerApiClientError
...
class HomeLedgerDataUpdateCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            stored = await Store(self.hass, 1, f"{DOMAIN}.{self.config_entry.entry_id}.bills").async_load()
            ...
            data["bills"] = stored["bills"] if stored is not None else []
            return data
